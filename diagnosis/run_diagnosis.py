...
def ensure_provider_used_column(connection: sqlite3.Connection) -> None:
    """Migrate a Phase 0 database created before provider_used existed."""
    columns = {
        row[1]
...
def main() -> None:
    ...
    try:
        ...
        delay_seconds = configured_delay_seconds()
        primary_count = 0
        fallback_count = 0
        default_count = 0
        ...
        for risk_event in risk_events:
            ...
            if result["used_default"]:
                ...
                primary_count += 1
            print(f"[{provider_used}] risk_id={risk_event['risk_id']} diagnosed")

        print("Diagnosis summary:")
        print(f"  - primary provider ({primary_provider}): {primary_count}")
        print(f"  - fallback provider: {fallback_count}")
        print(f"  - hardcoded default: {default_count}")
    finally:
        connection.close()
